...
```mermaid
flowchart LR
    P["ChikaProject.json"] --> EC["EngineContext"]
    S["Source file"] --> DB["AssetDatabase"]
    M[".meta"] <--> DB
    DB --> IR["ImporterRegistry"]
    IR --> IP["Imported path / artifact"]
    IP --> L["Loader / Decoder"]
    L --> AD["CPU AssetData"]
    AD --> AH["Asset Handle + SlotMap cache"]
    AH --> RM["ResourceManager"]
    RM --> RH["RHI objects + Resource Handle"]
    RM --> UQ["Upload request queue"]
    UQ --> RG["RenderGraph Upload Pass"]
    RG --> GPU["GPU-visible bytes"]
    RH --> RW["RenderWorld Proxy / Snapshot"]
    GPU --> RP["RenderPipeline draw"]
    RW --> RP
```
