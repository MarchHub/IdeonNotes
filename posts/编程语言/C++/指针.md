...
上述是错误代码,注意`swap`的函数签名`void swap(TreeNode* a, TreeNode* b)`

所以传入的是指针变量的副本,**变量本身的副本**,所以在里面做修改,则只会对局部变量进行修改,而不会影响外部实参.

所以正确的签名为`void swap(TreeNode*& a, TreeNode*& b)

按照引用传递传递指针变量,这样在修改指针变量本身的时候也保证修改成功.