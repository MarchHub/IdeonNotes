...
中间件的控制流 ——
假设定义了 `c *gin.Context`，则有 ——

- `c.Next()` 继续执行后续中间件和最终 Handler。
- `c.Abort()` / `c.AbortWithStatus(code)` 停止执行剩余中间件及 Handler，但不会结束当前函数，通常还需要 `return`。
- `c.AbortWithStatusJSON(code, obj)` 一步设置状态码、JSON 响应并中止后续处理，但不会结束当前函数。
...
只需要简单地使用 `r.POST("/post", PostFunction)` 即可把 `/post` 绑定上 `PostFunction`，非常地方便
...
其中，`gin.Context` 包含请求和响应的所有信息和方法，非常地好使
...
`c.JSON`，非常地常用！把自己的数据序列化成 `JSON` 形式，然后再写入 `HTTP Response`
...
顺带一提——`gin.H` 其实就是 `map[string]any`
